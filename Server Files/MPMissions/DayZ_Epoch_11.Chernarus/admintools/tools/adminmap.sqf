...
if (isNil "shz") then {shz = false;};  		//Show zombies on start
...
if (isNil "mgpsm") then {mgpsm = false;};  	//Move gps and remove it's frame
...
if (isNil "shPLT") then {shPLT = false;};		//Show PLT on start
if (isNil "shSL") then {shSL = false;};		//Show SL on start
if (isNil "shEB") then {shEB = false;};		//Show EB on start

mautoaddmap = false;				//Add map on start (GPS or MAP nesessary for functionality)
mautoaddgps = true;				//Add GPS on start (GPS or MAP nesessary for functionality)
...
_mapside = 20000; //15000				//Size of map (longest x or y) in meters
...
moptions = 
{
	...
	[
		...
		[format["Show plotpoles: %1",shPLT], [10], "", -5, [["expression", "shPLT = !shPLT;"]], "1", "1"],
		[format["Show safes and lockboxes: %1",shSL], [11], "", -5, [["expression", "shSL = !shSL;"]], "1", "1"],
		[format["Show event boxes: %1",shEB], [12], "", -5, [["expression", "shEB = !shEB;"]], "1", "1"]
	];  
...
if (isnil "mapm") then 
{
	...
	titleText ["Loading objects...","PLAIN DOWN"];titleFadeOut 2;
	helicrashes = nearestObjects [[maphalf,maphalf],["UH1Wreck_DZ","UH60Wreck_DZ","Mass_grave_DZ","Misc_cargo_cont_net1"],mapscanrad]; //"Misc_cargo_cont_net3","Misc_cargo_cont_tiny",
	tents = nearestObjects [[maphalf,maphalf],["TentStorage","TentStorageDomed","TentStorageDomed2"],mapscanrad];
	PLT = nearestObjects [[maphalf,maphalf],["Plastic_Pole_EP1_DZ"],mapscanrad];
	SL = nearestObjects [[maphalf,maphalf],["VaultStorage","VaultStorageLocked","LockboxStorage","LockboxStorageLocked"],mapscanrad];
	EB = nearestObjects [[maphalf,maphalf],["SpecialWeaponsBox","UNBasicWeapons_EP1","GuerillaCacheBox"],mapscanrad];
...
drawic = 
{
	if (!isnil "mapm") then 
	{
		...
		if shPLT then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\markers\waypoint.paa", tmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach PLT;}; //City
		if shSL then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\markers\waypoint.paa", tmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach SL;}; //Strongpoint
		if shEB then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\markers\waypoint.paa", tmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach EB;}; //Depot
