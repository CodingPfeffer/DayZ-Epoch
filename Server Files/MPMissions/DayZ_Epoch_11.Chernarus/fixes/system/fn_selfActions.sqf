...
///////////////////////////////////////////////////////////////////////////
    //Self Bloodbag by Krixes and Co
...
//beerdigen
	if (!_isAlive and !_isZombie and !_isAnimal and _hasETool and _isMan and _canDo) then {
        if (s_player_bury_human < 0) then {
            s_player_bury_human = player addAction [format["Bury Body"], "fixes\bury_human.sqf",cursorTarget, 3, true, true, "", ""];
        }
    } else {
	...
	*/
	...
	if ((dayz_myCursorTarget != _cursorTarget) and _isVehicle and !_isMan and _hasToolbox and (damage _cursorTarget < 1) and !_isDisallowRepair) then {
		if (s_player_repair_crtl < 0) then {
			...
			_menu1 = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_SALVAGEV", "fixes\salvage_vehicle.sqf",_cursorTarget, 0, true, false, "",""];
...
// custom crafting menu system change 
		//if ( cursorTarget isKindOf "WorkBench_DZ" && (player distance cursorTarget) < 5 && _canDo && !crafting_menu_open) then {
...
if((speed player <= 1) && hasSecondary && _canDo) then {
    if (s_player_suicide < 0) then {
        s_player_suicide = player addaction[("<t color=""#ff0000"">" + ("Selbstmord! Suicide!") +"</t>"),"fixes\suicide.sqf",_handGun,0,false,true,"", ""];
    };
} else {
